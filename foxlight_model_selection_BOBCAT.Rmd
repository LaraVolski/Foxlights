...
full.model <- lmer(Detections ~ Foxlight + Sheep_Presence + Dogs_Scaled + Elevation + 
                       Rugged.121 + Dist.Water + offset(log(Operation)) + (1|Camera),
                      data = rai_bobcat2, na.action = "na.fail")

AIC(full.model) ## AIC is 451.9571
vif(full.model) ## All vifs are under 4 

full.model.reduced <- glmer(attack ~ fence_ht_num + I(boun_dist^2) + 
                               rugged9 + rugged81 + 
                               I(water_dist^2) + vegbor_crs + NDVI_of_yr + chaparral + vernal_pool +
                               acres_dbl + avg_dogs + 
                               (1|pasture_name), 
