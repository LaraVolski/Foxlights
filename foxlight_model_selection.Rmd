...
Export data
```{r}
CALA_data <- filter(rai, Species == "CALA") %>% 
  select(Camera, Phase, Detections, Operation, RAI, Foxlight, Sheep_Presence) %>% 
  drop_na %>% 
  rename(Coyote_Detections = Detections,
         Days_Operating = Operation,
         Relative_Activity_Index = RAI)

head(CALA_data)

write.csv(CALA_data, "Volski-data.csv", row.names = F)
```
