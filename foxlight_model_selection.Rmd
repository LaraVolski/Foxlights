...
AIC = 431.4
```{r}
fit6 <- glmer.nb(Detections ~ Foxlight + Phase + offset(log(Operation)) + (1|Camera), data = subset(rai, Species == "CALA"))
summary(fit6)
```
### Foxlight * Phase
AIC = ?
...
```{r}
fit8 <- glmer.nb(Detections ~ vegetation.coarser.clean +  Phase + offset(log(Operation)) + (1|Camera), data = subset(rai, Species == "CALA"))
summary(fit8)
```
...
```{r}
fit4 <- glmer.nb(Detections ~ vegetation.coarser.clean + offset(log(Operation)) + (1|Camera), data = subset(rai, Species == "CALA"), control=glmerControl(optimizer="bobyqa"))
summary(fit4)
```
...
AIC = 425.5
```{r}
fit5 <- glmer.nb(Detections ~ fence.dist.clean + offset(log(Operation)) + (1|Camera), data = subset(rai, Species == "CALA"), control=glmerControl(optimizer="bobyqa"))
summary(fit5)
```

## Phase and Foxlight only
AIC = 435.3
```{r}
fit7 <- glmer.nb(Detections ~ Foxlight * Phase + offset(log(Operation)) + (1|Camera), data = subset(rai, Species == "CALA"))
summary(fit7)
