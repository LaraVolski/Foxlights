...
## Sheep & foxlight interaction
AIC = 427.5
```{r}
fit3 <- glmer.nb(Detections ~ Foxlight * Sheep_Presence + offset(log(Operation)) + (1|Camera), data = subset(rai, Species == "CALA"))
summary(fit3)
```
...
AIC = 431.1
```{r}
fit6 <- glmer.nb(Detections ~ Phase + offset(log(Operation)) + (1|Camera), data = subset(rai, Species == "CALA"), control=glmerControl(optimizer="bobyqa"))
summary(fit6)
```
