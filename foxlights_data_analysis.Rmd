...
# The mult_format multiplies the coyote RAI by 35, so instead of looking at daily average RAI, we are looking at it per 5 week phase. 

mult_format <- function() {
     function(x) format(50*x,digits = 2)}
...
ggplot(subset(rai, Species == "CALA"), aes(x = Foxlight, y = RAI, fill = Foxlight)) +
 ...
 # geom_jitter() +
  ...
  scale_fill_manual(values=c("antiquewhite1", "cadetblue1")) + 
...
multilines2 <-function (A, B, xscale = 24, linetype = c(1, 1), linecol = c("cadetblue3", 
                                                                           "antiquewhite3"), n.grid = 128, kmax = 3, adjust = 1,
                        ...) 
{
...
legend('top', c("Yes Foxlight", "No Foxlight"), col = c("cadetblue3", "antiquewhite3"), lwd = 3, lty=c(1,1), bty='n')
```
...
## Detections by Week
```{r}
detections.by.week <- read.csv("week_operation_dates.csv")
