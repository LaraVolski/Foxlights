...
```{r}
ggplot(subset(rai, Species == "LYRU"), aes(x = Sheep_Presence, y = RAI*35, by = Camera)) +
  geom_boxplot()
  
  
  
  ggplot(subset(rai_bobcat, Species == "LYRU"), aes(x = Sheep_Presence, y = RAI*35, fill = Sheep_Presence, by = Camera)) +
  geom_boxplot(alpha=2) +
  theme_bw() +
  theme(legend.position = "none") +
  scale_x_discrete(labels = c("No", "Yes")) +
